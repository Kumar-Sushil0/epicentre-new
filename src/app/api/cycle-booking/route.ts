  ...
  try {
    ...
    if (emailUser && emailPass) {
      ...
      if (answers && answers.length > 0 && questions && questions.length > 0) {
        ...
        `;
          responsesText = `\n\nPre-Booking Responses:\n${validResponses.map((item: { question: string; answer: string; idx: number }) => `Q${item.idx + 1}: ${item.question}\nA: ${item.answer}`).join("\n\n")}`;
        }
      }
          ...
          <div style="background-color: white; padding: 30px; margin-top: 20px; border-radius: 5px;">
            ...
            <div style="margin: 20px 0; padding: 15px; background-color: #f0f0f0; border-radius: 5px;">
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div><p style="margin: 0; color: #666; font-size: 13px;"><strong>Check-in:</strong></p><p style="margin: 5px 0 0 0; color: #1a1a1a; font-size: 15px;">${formatDate(checkIn)}</p></div>
                <div><p style="margin: 0; color: #666; font-size: 13px;"><strong>Check-out:</strong></p><p style="margin: 5px 0 0 0; color: #1a1a1a; font-size: 15px;">${formatDate(checkOut)}</p></div>
              </div>
            </div>
            <div style="margin: 20px 0;"><p style="margin: 0; color: #666; font-size: 14px;"><strong>Name:</strong></p><p style="margin: 5px 0 0 0; color: #1a1a1a; font-size: 16px;">${name}</p></div>
            <div style="margin: 20px 0;"><p style="margin: 0; color: #666; font-size: 14px;"><strong>Email:</strong></p><p style="margin: 5px 0 0 0; color: #1a1a1a; font-size: 16px;"><a href="mailto:${email}" style="color: #d4af37; text-decoration: none;">${email}</a></p></div>
            <div style="margin: 20px 0;"><p style="margin: 0; color: #666; font-size: 14px;"><strong>Phone:</strong></p><p style="margin: 5px 0 0 0; color: #1a1a1a; font-size: 16px;"><a href="tel:${phone}" style="color: #d4af37; text-decoration: none;">${phone}</a></p></div>
            ${notes ? `<div style="margin: 20px 0;"><p style="margin: 0; color: #666; font-size: 14px;"><strong>Additional Notes:</strong></p><div style="margin: 10px 0 0 0; padding: 15px; background-color: #f9f9f9; border-radius: 5px; color: #1a1a1a; font-size: 16px; line-height: 1.6; white-space: pre-wrap;">${notes}</div></div>` : ""}
            ${responsesHTML}
          </div>
