...
export default function CarouselCard({
    ...
    const CardContent = (
        <div
            ref={cardRef}
            className={`group relative aspect-[25/20] overflow-hidden bg-gold-500 border border-earth-800/50 shadow-xl rounded-[10px] ${className} ${href ? 'cursor-pointer' : ''}`}
            ...
            <div className="absolute inset-0 z-10 bg-earth-900">
                ...
                >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                        <h4 className={`text-[21px] font-normal ${overlayColor === 'gold-solid' && shouldShowHoverState ? 'text-earth-900' : 'text-[#e7dfd3]'}`} style={{ fontFamily: 'Outfit, sans-serif', fontSize: titleSize, textShadow: overlayColor === 'gold-solid' && shouldShowHoverState ? 'none' : '0 2px 8px rgba(0, 0, 0, 0.8), 0 4px 16px rgba(0, 0, 0, 0.6)' }}>{hoverTitle || title}</h4>
                        {href && !shouldShowHoverState && (
                            <span className="material-symbols-outlined text-gold-500 text-xl">arrow_forward</span>
                        )}
                        {socialIcons && shouldShowHoverState && (
